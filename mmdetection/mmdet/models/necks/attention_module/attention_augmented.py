...
class AugmentedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dk=40, dv=4, Nh=4, shape=16, relative=False, stride=4):
...
class AugmentedAttention(nn.Module):
    def __init__(self, levels, in_channels, out_channels):
        ...
        for _ in range(levels):
            self.augmented.append(AugmentedConv(in_channels, out_channels))
    ...
    def forward(self, x) :
        ...
        attention_maps = [self.augmented[j](x) for j in range(self.levels)]
